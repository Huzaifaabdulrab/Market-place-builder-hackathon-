...
async function getData(id: string): Promise<Car | null> {
  const fetchData = await client.fetch(`*[_type == 'car' && _id == $id][0]`, { id });
...
export default async function BlogContent({ params }: { params: { id: string } }) {
  ...
  const { name, type, fuelCapacity, transmission, seatingCapacity, pricePerDay, image, originalPrice } = blog;
  
  const imageUrl = image?.asset?._ref ? urlFor(image.asset._ref).url() : "/images/fallback.png"; 
            ...
            <div className="lg:ml-40 screenLeft lg:w-[1140px] lg:h-full lg:flex flex-col">
              ...
              <div className="flex flex-col lg:flex-row p-6 rounded-lg">
                ...
                <div className="flex flex-col items-center lg:items-start w-full lg:w-1/2">
                  {/* Large Image */}<div className="w-full h-auto lg:w-[492px] lg:h-[360px] mb-6 ml-4">
  <Image
    src={imageUrl}
    alt={`${name} Image`}
  ...
  />
                  ...
                  <div className="flex gap-5 w-[328px] h-[64px] lg:w-full lg:mr-[7.5%] ml-2">
                    <div className="lg:w-[148px] lg:h-[124px] w-[96px] h-[96px]">
                      <Image src={imageUrl} alt="Small Image 1"
                         width={148} 
                         height={124}                  
                         objectFit="cover"    
                      className="w-full h-full object-fill rounded-lg" />
                ...
                <div className="w-[328px] h-[408px] lg:ml-1 ml-auto mt-20 lg:mt-0 lg:w-[492px] lg:h-auto bg-white shadow-md rounded-lg p-6">
                  {/* Title */}
                  <h3 className="text-[28px] lg:text-[34px] font-bold text-gray-900 mb-2">{name}</h3>
                  ...
                  <p className="text-gray-700 lg:text-[20px] text-[12px] mb-6">
                    {name} has become the embodiment of {name}’s outstanding performance, inspired by the most unforgiving proving ground, the “race track”.
                  </p>

                  {/* Specifications */}
                  <div className="grid grid-cols-2 gap-x-6 gap-y-4 text-gray-600 text-[10px] lg:text-[20px] mb-8">
                    <div>
                      <span className="font-bold text-labelTxt">Type Car:</span>{type}
                    </div>
                    <div>
                      <span className="font-bold text-labelTxt">Capacity:</span> {seatingCapacity}
                    </div>
                    <div>
                      <span className="font-bold text-labelTxt">Steering:</span> {transmission}
                    </div>
                    <div>
                      <span className="font-bold text-labelTxt">Gasoline:</span> {fuelCapacity}
                    </div>
                  </div>
                  ...
                  <div className="flex sm:flex-row items-start justify-between">
                    <span>
                      <span className="text-[20px] lg:text-[24px] font-bold text-gray-900">{pricePerDay}/</span>
                  ...
                  <p className="text-[18px] text-gray-500 line-through ml-2">{originalPrice}</p>
                </div>
              </div>
            </div>
